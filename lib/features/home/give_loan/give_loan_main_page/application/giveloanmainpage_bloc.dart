...
part 'giveloanmainpage_event.dart';
part 'giveloanmainpage_state.dart';
part 'giveloanmainpage_bloc.freezed.dart';

@injectable
class GiveloanmainpageBloc
    extends Bloc<GiveloanmainpageEvent, GiveloanmainpageState> {
  final LoanRepo<LoanMainInfo> loanRepo;
  GiveloanmainpageBloc(this.loanRepo) : super(GiveloanmainpageState.initial()) {
    on<GiveloanmainpageEvent>(
      (event, emit) async {
        await event.map(
          initial: (e) async {
            emit(state.copyWith(isSubmitting: true));
            final res =
                await loanRepo.getMainInfo(loanType: LoanType.investment);
            res.fold(
              (l) {
